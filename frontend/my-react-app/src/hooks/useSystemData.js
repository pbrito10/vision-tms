...
export function useSystemData() {
  ...
  const applyLiveUpdate = useCallback((liveUpdate) => {
    ...
    if (liveUpdate.metrics) {
      setMetrics(liveUpdate.metrics)
    }
    setError(null)
    setIsLoading(false)
  }, [])
  ...
  const refreshLive = useCallback(async () => {
    const [nextStatus, nextProgramState, nextMetrics] = await Promise.all([
      apiClient.getStatus(),
